document.addEventListener('DOMContentLoaded', () => {
  ...
  const L = {
    ja: {
      ...
      'footer':'設定は自動で保存されます。','msg.pickTime':'時刻を選択してください','msg.invalidTime':'不正な時刻です','msg.alarmSound':'アラームが鳴りました'
    },
    en: {
      ...
      'footer':'Settings are saved automatically.','msg.pickTime':'Please pick a time','msg.invalidTime':'Invalid time','msg.alarmSound':'Alarm'
    }
  };
  ...
  let showSeconds = localStorage.getItem('nclock_show_seconds')==='true' ? true : true;
  ...
  function applyLocalization(){
    ...
    document.querySelectorAll('[data-key-start]').forEach(el=>{
      el.textContent = running? t('btn.stop'): t('btn.start');
    });
  ...
  function playAlarmSound(){
    try{
      ...
      for(let i=0;i<6;i++){
        const o=ctx.createOscillator(); o.type='sine'; o.frequency.value=880-i*40;
      ...
      alarms.forEach(a=>{
        ...
        if(a.hour===nowReal.getHours() && a.min===nowReal.getMinutes()){
          if(lastTriggered!==keyNow){
            ...
            try{if(Notification.permission==='granted'){new Notification('N Clock',{body:`${t('msg.alarmSound')}: ${String(a.hour).padStart(2,'0')}:${String(a.min).padStart
